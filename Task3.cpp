...
public:
	...
	virtual void write(ostream& wr)const {
	...
	virtual void display() {
		...
		cout << "the Second Name is :" << secondName << endl;
...
public:
	...
	bool operator==(const Teacher& other) {
		if (Person::operator==(other) || this->teacherSalary != other.teacherSalary ||
			...
			this->teacherDestination != other.teacherDestination
			|| this->teacherDateOfJoining != other.teacherDateOfJoining) {
			return false;
		}
		else {
			return false;
...
public:
	...
	void _teacher() {
		int _id;
		cout << "Enter the Id of teacher You Want to find :" << endl;
		cin >> _id;
		for (int i = 0; i < numDegree; i++) {
